...
syms x y;
I_anal = double(int(int(x*exp(x*y),x,0,3),y,-1,1)) % analytical answer, converted to double
return
...
function [I] = dbl_trap(f,ax,bx,ay,by,n) % copy trapezoidal code
I=0;
h=(by-ay)/n;
for i=0:n
    y=ay+i*h;
    if (i == 0 || i == n)
        I=I+trap(f,y,ax,bx,n); % trap is your integral
    else
...
function [I] = dbl_simp(f,ax,bx,ay,by,n) % double integral
I=0;
h=(by-ay)/n;
for i=0:n
    y=ay+i*h;
