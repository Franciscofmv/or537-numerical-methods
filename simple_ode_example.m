...
function [y] = euler(f,x,y0) % slope evualted at xi
n = length(x);
y=zeros(1,n);
y(1) = y0;
for i=1:n-1
...
for i=1:n-1
    ...
    h=x(i+1)-x(i);
    k1=f(xi,yi);
    k2=f(xi+h,yi+k1*h); % slope evualted at xi+h, prediction
    y(i+1)=yi+h*(k1+k2)/2;% take the average of the slopes, correctness
end
return
